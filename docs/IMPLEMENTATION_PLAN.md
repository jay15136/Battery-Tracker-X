...
|---|---|---|---|
| 2026-09-09 | Windows resolves application-managed storage next to the executable (`ExecutableRelativeAppDataDirectoryService`), not the per-user profile | User request: run Battery Tracker fully portably from a USB drive, with one Windows PC's data usable unmodified from any other PC the drive is plugged into. | The root is computed from `Platform.resolvedExecutable` at every launch, so it is correct regardless of drive letter. Existing per-user AppData installs are not migrated automatically. Other platforms keep `PathProviderAppDataDirectoryService`, selected by `Platform.isWindows` in `lib/main.dart`. |
...
| ENV-006 | `drift_dashboard_repository_test.dart`'s "configured charge threshold and Set difference" test failed once the real calendar date passed the test's hardcoded `now` fixture (2026-09-08): an unrelated membership row's `added_at` used the schema's real-wall-clock `clientDefault` instead of the fixture, so it could land after the test's fixed `removed_at` write and trip the `removed_at >= added_at` CHECK constraint. | Broke one previously-passing test on 2026-09-09, unrelated to any feature change | Resolved | The insert now sets `addedAt: Value(now)` explicitly, anchoring it to the same fixed fixture as the later `removedAt` write so the test no longer depends on which real day it runs. |
| ENV-007 | Three pub cache package directories (`jni-1.0.3`, `file_selector_windows-0.9.3+5`, `path_provider_windows-2.3.0`) were left as empty stubs, all timestamped from the same incident, most likely a `flutter`/`dart` process force-killed mid-download during earlier hang investigation. Pub's cache-presence check only saw that the directories existed, so it never redownloaded them; `flutter build windows` failed compiling `jni`, and `file_selector_windows_plugin.dll`/`path_provider_windows` silently dropped out of Windows plugin registration without a build error. | Broke the Windows Release build; would have shipped a portable build with native file dialogs (Icon import, CSV export/import, Backup create/restore) silently non-functional | Resolved | Deleted the three empty directories and reran `flutter pub get` to redownload them cleanly; verified `file_selector_windows_plugin.dll` is present in the rebuilt output and the file-dialog-dependent features work. Found only by comparing the rebuilt Release folder's file list against a known-good earlier build — a build succeeding is not sufficient evidence a plugin's DLL was actually bundled. |
